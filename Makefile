...
container:
	...
	docker build -t $(REGISTRY)/policy-server-$(ARCH):$(CHECKSUM) --build-arg ARCH=$(ARCH) --build-arg GIT_TAG=$(GIT_TAG) --build-arg GIT_COMMIT=$(GIT_COMMIT) .
...
.PHONY: push
push: container
	docker tag $(REGISTRY)/policy-server-$(ARCH):$(CHECKSUM) $(REGISTRY)/policy-server-$(ARCH):$(GIT_TAG)
	docker push $(REGISTRY)/policy-server-$(ARCH):$(GIT_TAG)
...
push-multi-arch:
	docker manifest create --amend $(REGISTRY)/policy-server:$(GIT_TAG) $(shell echo $(ALL_ARCHITECTURES) | sed -e "s~[^ ]*~$(REGISTRY)/policy-server\-&:$(GIT_TAG)~g")
	@for arch in $(ALL_ARCHITECTURES); do docker manifest annotate --arch $${arch} $(REGISTRY)/policy-server:$(GIT_TAG) $(REGISTRY)/policy-server-$${arch}:${GIT_TAG}; done
	docker manifest push --purge $(REGISTRY)/policy-server:$(GIT_TAG)
...
test-image: container
	IMAGE=$(REGISTRY)/policy-server-$(ARCH):$(CHECKSUM) EXPECTED_ARCH=$(ARCH) EXPECTED_VERSION=$(GIT_TAG) ./test/test-image.sh
...
update-generated:
	...
	$(GOPATH)/bin/openapi-gen -i sigs.k8s.io/wg-policy-prototypes/policy-report/pkg/api/wgpolicyk8s.io/v1alpha2,k8s.io/apimachinery/pkg/apis/meta/v1,k8s.io/apimachinery/pkg/api/resource,k8s.io/apimachinery/pkg/version,k8s.io/api/core/v1.ObjectReference -p pkg/api/generated/openapi/ -O zz_generated.openapi -o $(REPO_DIR) -h $(REPO_DIR)/scripts/boilerplate.go.txt -r /dev/null
